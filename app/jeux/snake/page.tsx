...
export default function Snake() {
  ...
  };window.addEventListener("keydown",onKey);return()=>window.removeEventListener("keydown",onKey);},[dir,alive]);

  useEffect(()=>{if(!alive) return;const id=setInterval(()=>{
    setSnake(sn=>{
      ...
      const ns=[head,...sn]; if(!eat){ns.pop();}else{setScore(s=>s+1);let fx=Math.floor(Math.random()*COLS),fy=Math.floor(Math.random()*ROWS);
        while(ns.some(p=>p.x===fx&&p.y===fy)){fx=Math.floor(Math.random()*COLS);fy=Math.floor(Math.random()*ROWS);} setFood({x:fx,y:fy});}
      return ns;
    });
  },120);return()=>clearInterval(id);},[dir,alive,food]);
  ...
  useEffect(()=>{const c=canvasRef.current;if(!c) return;const ctx=c.getContext("2d")!;
    ...
    ctx.fillStyle="#0f0";snake.forEach(p=>ctx.fillRect(p.x*SIZE,p.y*SIZE,SIZE-1,SIZE-1));
    ctx.fillStyle="#f33";ctx.fillRect(food.x*SIZE,food.y*SIZE,SIZE-1,SIZE-1);
  },[snake,food]);

  function reset(){setSnake([{x:5,y:10},{x:4,y:10}]);setDir({x:1,y:0});setFood({x:12,y:10});setScore(0);setAlive(true);}

  return (
    <main className="px-6 py-20 text-white">
      <h1 className="mb-2 text-3xl font-bold">Jeu : Snake</h1>
      <p className="mb-4 text-white/80">Flèches ← ↑ → ↓.</p>
      <div className="mb-4 flex items-center gap-4"><div>Score : <b>{score}</b></div>{!alive&&(
