...
const SIZE = 20;     // taille d'une case en px
const COLS = 20;
const ROWS = 20;

type Pt = { x: number; y: number };

export const metadata = { title: "Snake — Jeux" };
...
export default function SnakePage() {
  ...
  // Focus auto pour capter le clavier
  useEffect(() => {
    wrapRef.current?.focus();
  }, []);

  // Clavier (avec preventDefault pour éviter le scroll)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setSnake((sn) => {
        ...
        // auto-collision : ignorer la dernière case si on ne mange pas (la queue recule)
  ...
  return (
    <main className="px-6 py-20 text-white">
      <div
        ref={wrapRef}
        tabIndex={0}
        className="mx-auto max-w-fit outline-none"
        aria-label="Surface de jeu Snake"
      >
        <div className="mb-3 flex items-center gap-4">
          ...
          <div className="text-white/60 text-sm">(Espace = Pause)</div>
        </div>
