...
export default function AdminPage() {
  ...
  const nextUrl = sp?.get("next") ?? "";   // ✅ plus d’erreur TS
    ...
    <div className="mx-auto max-w-3xl p-4 text-white">
      ...
      {!me ? (
        ...
        <div className="rounded-2xl border border-white/20 bg-black/40 p-4 backdrop-blur">
          <div>Connecté en tant que : <strong className="capitalize">{me.role}</strong></div>
          <button onClick={logout} className="mt-2 rounded-full border border-white/30 px-3 py-1.5 hover:bg-white/10">Se déconnecter</button>
        </div>
      )}
    </div>
  );
